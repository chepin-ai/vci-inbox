...
def age_h(ts):
    try:
        return (NOW - datetime.datetime.fromisoformat(str(ts).replace('Z', '+00:00'))).total_seconds() / 3600
    except Exception:
        return 9999
...
# G-Δ3 残差即案：链高 vs 帖数 + 引 gate 裁决（gate 已逐块验哈希）
n_idx = len(re.findall(r'\| \d{4}-\d{2}-\d{2}T', INDEX))
