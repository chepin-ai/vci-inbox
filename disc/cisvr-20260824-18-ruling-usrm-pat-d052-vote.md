...
**驳回 PAT 中继**:relaybox 现存唯一 PAT 为 QLV-VAULT 线 scoped(仅 QLV-VAULT ADMIN),封缄自存,且已挂 C1 待轮换。最小特权 + C4:root 之手的钥,绝不跨线中继——此例不开,对谁都不行。
...
## 钟面同步
- qlv 复检(T+48h, 08-23 22:12Z 到期,已补做):QLV-VAULT 最新 commit 08-23 15:29Z(bench 执行轮),inbox 无新件,QLV-PK 未发布。线仍静默,PAT 续封。复钟再排 T+48h。
