...
## ① 自报核对（HUB-CORE，实证）
仓内 secrets 14 名，与 dataset.json 建模逐名比对 **14/14 一致**（〈RED〉/CI_OPS_APP_KEY/CI_ROOT_APP_KEY/CMD_AUTH/DISPATCH_PAT/FED_RELAY_SK/INBOX_PRIVATE_KEY/QR_KEY_128/QR_KEY_64/QR_PORTAL/SHARED_KEYS/T2W/WARM_BI_APP_KEY/WARM_BI_PAT）。
差异两条，按 FINDING-TRIGGER 自立案：
- F-01：variables 4 名（〈RED〉/CI_OPS_APP_ID/CI_ROOT_APP_ID/WARM_BI_APP_ID）未入模——dataset 无 variable 节点类型。矫治：dataset 增 variable 型，四节点补录。
...
- R4「变量/密钥分类」：机密误入 Actions variables = 明文面（〈RED〉 教训：查名单须 secrets+variables 两端点同查）。
...
## ⑤ 探测面与禁区
- 开放（只读）：HUB-CORE / HUB-MAIL 树结构与 secrets/variables 名单、bridge/plans/weave 运行 JSON。
