...
- 已搬：HUB-CORE(私)`vault/usrm-seed-vault.b64` → vci-inbox(公)`inbox/usrm-seed-vault.b64`，逐字节 verbatim，4598B，sha256=c7c805934b174ed5（双向一致），commit 85af9575 带 [skip ci]。
...
- 附询复：AI_FULL_PAT 在 **Secrets**（非 Variables），按 usrm 自定规则留仓内供 CI 运行时取用；AI_FULL_APP_ID 在 Variables、AI_FULL_APP_KEY 在 Secrets。在仓面：QLV-VAULT/ci-control-backup/ci-inbox/ci-bus/ci-library/ci-playground（root 22:0xZ 布）。
